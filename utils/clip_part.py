...
from utils.ift_block_feature import ContraNorm
    ...
    def transformer_deep(self, x, deep_x):
        if self.deep:
            if deep_x.dim == 2:
                if self.deep_layer == None:
                    deep_x = deep_x.unsueeze(0).expand(self.num_layer - 1, -1, -1)  # all layers exsit cdu
                else:
                    deep_x = deep_x.unsueeze(0).expand(self.deep_layer[1] - self.deep_layer[0] + 1, -1, -1) # only specified layers exsit cdu
        ...
        for i in range(self.num_layer):
            if i == 0:
                ...
                if self.deep:
                    if self.deep_layer == None:   # only specified layers exsit cdu
                        if i <= deep_x.shape[0]:   
...
class ImageEncoder_Conv(nn.Module):
    ...
    def forward(self, x, vctx=None, deep_vctx=None, return_feat=False):
        '''
        Maybe futherly there will be some cdu tuning methods for convolutional networks
        '''
        def stem(x):
